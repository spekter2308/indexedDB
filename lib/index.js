...
const { insertDeviceValue, average, median } = require('./utils.js');
            ...
            if (devicesData.has(timeRange)) {
                ...
                for (const deviceInfo of groupDevicesByLocation) {
                    if (devicesForDateGroup.has(deviceInfo.id)) {
                        ...
                        devicesDataTable.push([deviceName, deviceValues.min, deviceValues.max, deviceValues.avg.toFixed(3), deviceValues.median])
                    }
                }
            }
